...
Dependabot continues to update Go modules and GitHub Actions. Dependabot does
not own the `go` or `toolchain` directive.

Start with [`examples/workflows/update-go.yml`](../examples/workflows/update-go.yml).
...
Default the workflow to read-only permissions. Expose credentials with contents
and pull-request write access only to the PR creation step.
